                    ...
                    <form method="POST" action="{{ route('salas.update', $sala->id) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-input id="fecha_hora" class="block mt-1 w-full"
                                     type="datetime-local"
                                     name="fecha_hora"
                                     :value="$sala->fecha_hora->format('Y-m-d\TH:i:s')"
                                     required />
                        </div>
                            ...
                            @else
                                <x-select id="paciente_id" name="paciente_id" required>
                                    ...
                                    @foreach ($pacientes as $paciente)
                                        <option value="{{$paciente->id}}" @if ($sala->paciente_id == $paciente->id) selected @endif>{{$paciente->user->name}} ({{$paciente->nuhsa}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
                            ...
                            @else
                                <x-select id="medico_id" name="medico_id" required>
                                    ...
                                    @foreach ($medicos as $medico)
                                        <option value="{{$medico->id}}" @if ($sala->medico_id == $medico->id) selected @endif>{{$medico->user->name}} ({{$medico->especialidad->nombre}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <table class="min-w-max w-full table-auto">
                        ...
                        @foreach ($sala->medicamentos as $medicamento)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center">
                                        ...
                                        <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                            <form id="detach-form-{{$sala->id}}-{{$medicamento->id}}" method="POST" action="{{ route('salas.detachMedicamento', [$sala->id, $medicamento->id]) }}">
                                                @csrf
                                                @method('delete')
                                                <a class="cursor-pointer" onclick="getElementById('detach-form-{{$sala->id}}-{{$medicamento->id}}').submit();">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('salas.attachMedicamento', [$sala->id]) }}">
                        @csrf
